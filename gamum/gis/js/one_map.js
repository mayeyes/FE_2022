function init(){
	...
	if($("#feeling_msg").size() !=0){feeling_msg();}
}
...
function data_tab_AC(){
	...
	_set(0);
    if($("#header #gnb ul li a.on").parent().hasClass("damages")){
        _set(3);
    }
    ...
    $("#data_bx .tab_bx .notibx .message_box>ul").css("grid-template-columns", "repeat(" + $("#data_bx .tab_bx .notibx .message_box>ul>li").size() + ",1fr)");
}
	...
	var obj = $("#step_bx>[data-name]");
		...
		btn.click(function(){
			if($(this).parent().hasClass("on")){
                ...
                if($(this).parents("#bind").find("#feeling_msg").hasClass("hide")){
                    
                }else{
                    $(this).parents("#bind").find("#feeling_msg").addClass("hide");
                }
			}
			return false;
		});
...
function setMaxHeight(){
	let max_height = 0;
	let liTags = '.notibx .message_box :visible li';
	$(liTags).each(function(){
		let this_height = parseInt($(this).css('height'));
...
function feeling_msg(){
    var btn = $("#feeling_msg .close");
    btn.html("<span>닫기</span>");
    $("#feeling_msg .notibx .message_box >strong").click(function(){
        btn.trigger("click");
    });
    btn.click(function(){
        if($(this).parent().hasClass("hide")){
            if($("#step_bx>[data-name]").hasClass("on")){
                $("#step_bx>[data-name]").removeClass("on");
            }
            $(this).parent().removeClass("hide");
			$(this).parent().find(".notibx .message_box>ul").show();
        }else{
            $(this).parent().addClass("hide");
            $(this).parent().find(".notibx .message_box>ul").hide();
            for(var i=0; i<$('#step_bx [data-name]').size(); i++){
                $('#step_bx [data-name]').eq(i).addClass("on");
            }
		}
     ...
     if($("body").width()<=1330){
        ...
        btn.parent().find(".notibx .message_box>ul").hide();
     }
    
    $("#feeling_msg .notibx .message_box>ul").css("grid-template-columns", "repeat(" + $("#feeling_msg .notibx .message_box>ul>li").size() + ",1fr)");
}
