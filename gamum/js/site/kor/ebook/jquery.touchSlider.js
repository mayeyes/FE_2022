	...
	var touchSlider = {
		...
		animate : function (d, btn_click, speed) {
			if(this._drag || !this._scroll || btn_click) {
				...
				var totalNum = this.get_page().total;
				var currentNum = this.get_page().current - 1;

				for(var i=0; i<totalNum; ++i) {
					this._list.eq(i).css({ "visibility" :"hidden" });
				}
				
				if ( currentNum == 0 ) {
					this._list.eq(totalNum -1).css({ "visibility" :"visible" });
					this._list.eq(0).css({ "visibility" :"visible" });
					this._list.eq(1).css({ "visibility" :"visible" });
				} else if ( currentNum == (totalNum -1) ) {
					this._list.eq(totalNum -2).css({ "visibility" :"visible" });
					this._list.eq(totalNum -1).css({ "visibility" :"visible" });
					this._list.eq(0).css({ "visibility" :"visible" });
				} else {
					this._list.eq(currentNum-1).css({ "visibility" :"visible" });
					this._list.eq(currentNum).css({ "visibility" :"visible" });
					this._list.eq(currentNum+1).css({ "visibility" :"visible" });
				}
