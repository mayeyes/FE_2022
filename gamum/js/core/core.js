...
function slide_type_01(slide){
	...
	//총 카운트 적용
    /*slide.counts = slide.controls.find(">.count");*/
	slide.counts.html("<strong>"+slide.nums+"</strong><span>/</span>"+slide.li.size()+"</span>");
    ...
    //체크
    function _check(){
    	// slide.ul.find(">li:not(:hidden)")
    	if(slide.ul.find(">li:hidden").size() > 0){
    		slide.ul.find(">li:hidden").remove();
    	} else {
    		slide.cloneVal.appendTo(slide.ul.empty());
    	}
        slide.li = slide.ul.find(">li");
        var w = slide.innerWidth();
        for(var i=0; i<slide.li.size(); i++){
            var liW = Number(slide.li.eq(i).position().left) + Number(slide.li.eq(i).width());
            if(w > liW){
                slide.li.eq(i).addClass("show");
            } else {
                slide.li.eq(i).removeClass("show");
            }
        }
    ...
    var saveCheckVal = "";
    $(window).resize(function(){
        clearTimeout(saveCheckVal);
        saveCheckVal = setTimeout(function(){
            _check();
            _num();
        },100);
    });
	...
	function movement(ty){
		...
		if(ty == "left"){
			slide.li.last().addClass("show").prependTo(slide.ul);
			slide.ul.css("left",w+"px");

			w = 0;
			slide.nums = slide.li.last().attr("data-count");
            
            slide.ul.find(">li.show").last().removeClass("show");
		} else {
			slide.nums = slide.li.eq(1).attr("data-count");
            
            slide.ul.find(">li.show").last().next().addClass("show");
		}
        
		slide.ul.stop().animate({"left":w+"px"},slide.speeds,function(){
			if(ty == "right"){
				slide.li.eq(0).removeClass("show").appendTo(slide.ul);
				slide.ul.css("left","0");
