...
var __popupzone = {
	set:function(){
		$('<div class="controll">\
			...
			<a href="#popupzones__prev" class="btn_prev"><span id="popupzones__prev">이전</span></a>\
			<a href="#popupzones__play" class="btn_play"><span id="popupzones__play">재생</span></a>\
			<a href="#popupzones__stop" class="btn_stop"><span id="popupzones__stop">정지</span></a>\
			<a href="#popupzones__next" class="btn_next"><span id="popupzones__next">다음</span></a>\
...
var popItemView = function(apbaId, reportFormRootNo, apbaName, reportGubun, disclosureNo){
	
	var sUrl = location.href;
	var sPrePath = "https://alio.go.kr";
	var wSize = {width:"1240",height:"950"};
    // var disclosureNo = ['21026','21025','21028','63795','63796'];
	if(!$(".js_mobile_check").is(":hidden")){
		sPrePath += "/mobile";
		wSize = {width:screen.width,height:screen.height};
	}
    if (reportGubun == 'SUSI') {
        wSize = {width:screen.width,height:screen.height};
    }
	
	if(reportGubun == "JUNG" || reportGubun=="1"){	// 정기보고서 일경우
		...
		if(reportFormRootNo == "63601") {
			...
			if(result == "true") {
				...
				//window.open(sPrePath+"/item/itemReportTerm.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo + "&disclosureNo=" + (disclosureNo || ''), "_blank", "scrollbars=no, resizable=yes, width="+wSize.width+", height="+wSize.height);
				s_urls = sPrePath+"/item/itemReportTerm.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo + "&disclosureNo=" + (disclosureNo || '');
			}
		} else {
            //window.open(sPrePath+"/item/itemReportTerm.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo + "&disclosureNo=" + (disclosureNo || ''), "_blank", "scrollbars=no, resizable=yes, width="+wSize.width+", height="+wSize.height);
			s_urls = sPrePath+"/item/itemReportTerm.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo + "&disclosureNo=" + (disclosureNo || '');
	...
	//수시보고서 -PC
		...
		//수시 보고서 - 모바일
		if(sPrePath != "https://alio.go.kr"){	
            this.formLocation("https://alio.go.kr/mobile/item/itemOrganListSusi.do?apbaId="+apbaId+"&reportFormRootNo="+reportFormRootNo);		
		}else{
            // var disclosureNo = ['21026','21025','21028','63795','63796'];
			// if(disclosureNo.indexOf(reportFormRootNo)){
			// 	window.open("/item/itemOrganList.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo, "_blank", "scrollbars=no, resizable=yes, top=0, width="+wSize.width+", height="+wSize.height);
			// }else{
				//var win = window.open("", "_blank", "scrollbars=no, resizable=yes, top=0, width="+wSize.width+", height="+wSize.height);
				
					//win.location.href = "https://alio.go.kr/item/itemOrganList.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo;
			   
				//win.location = "https://alio.go.kr/item/itemOrganList.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo;

				var s_urls = "https://alio.go.kr/item/itemOrganList.do?apbaId=" + apbaId + "&reportFormRootNo=" + reportFormRootNo;
...
var __history = {
	set:function(str){
		...
		$('<strong class="hidden">KORES 한국광물자원공사</strong>').prependTo(obj.pc.find('.midd [data-group="1"]'));
		$('<strong class="hidden">한국광해관리공단</strong>').prependTo(obj.pc.find('.midd [data-group="2"]'));
...
var __downloadBtnTitle = {
	set:function(str){
		var obj = str;
		if(obj.attr("title") === undefined){
			var t = obj.text().replace("다운로드","");
			obj.attr("title",t+" 다운로드");
		}
	},
	init:function(){
		for(var i=0; i<$('[data-btn][data-icon="5"]').length; i++){
			this.set($('[data-btn][data-icon="5"]').eq(i));
		}
	}
}
...
var _layout = {
	content:function(){
		...
		__history.init();//연혁
		__downloadBtnTitle.init();//다운로드 title 생성
		__userData.init();//개인정보처리절차
	},
	sub:function(){
		__gnb.init(); //상단 메뉴
