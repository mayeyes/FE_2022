...
/////////////////////////////////////////////////////////////////////////////////////////////////////////// */
$(function(){
	if($("#remote").length !== 0) Lmenu_setting(Lmenu_code);
});
...
function board_list_secret_AC(){
	...
	var htmls = '';
		...
		htmls += '	<div class="in">';
		htmls += '		<div class="head"><strong>게시물 권한확인</strong></div>';
		htmls += '		<div class="midd">';
		htmls += '			<div>';
		htmls += '				<ul>';
		htmls += '					<li><label for="sct_name">이름</label><input type="text" name="sct_name" id="sct_name" title="이름을 입력해주세요." /></li>';
		htmls += '					<li><label for="sct_password">비밀번호</label><input type="password" name="sct_password" id="sct_password" title="비밀번호를 입력해주세요." /></li>';
		htmls += '				</ul>';
		htmls += '				<button class="btn middle" onclick="board_secret_form();">확인</button>';
		htmls += '			</div>';
		htmls += '			<span>글작성시 입력하셨던 이름과 비밀번호를 입력해주세요.</span>';
		htmls += '		</div>';
		htmls += '		<a href="javascript:;" class="btn_close"><span>닫기</span></a>';
		htmls += '	</div>';
		htmls += '</div>';
...
ex)
<a href="#" onclick="sns.popup('f');return false;">페이스북</a>
<a href="#" onclick="sns.popup('t');return false;">트위터</a>
<a href="#" onclick="sns.popup('n');return false;">블로그</a>
<a href="#" onclick="sns.popup('b');return false;">밴드</a>
<a href="#" onclick="sns.popup('k');return false;">카카오스토리</a>

/////////////////////////////////////////////////////////////////////////////////////////////////////////// */
var sns = {
	update:function(){
		var nUrl,nWidth,nHeight,nLeft,nTop,nScroll;
		nUrl = arguments[0];
		nWidth = arguments[1];
		nHeight = arguments[2];
	...
	popup:function(_this){
		...
		if(type==='f'){
			p = '//www.facebook.com/sharer/sharer.php?u='+h+'&t='+t;
			this.update(p, 900, 450, 'no');
		} else if (type==='t') {
			p = '//twitter.com/home?status='+encodeURIComponent(t)+' '+h;
			this.update(p, 800, 400, 'yes');
		} else if (type==='k') {
			p = 'https://story.kakao.com/share?url='+h;
			this.update(p, 900, 450, 'no');
		} else if (type==='b') {
			p = 'http://www.band.us/plugin/share?body='+encodeURIComponent(t)+'%0A'+h;
			this.update(p, 800, 400, 'yes');
		} else if (type==='n') {
			p = "http://share.naver.com/web/shareView.nhn?url="+h+"&t="+encodeURIComponent(t);
			this.update(p, 800, 400, 'yes');
		} else if (type==='l') {
			...
			if(location.href.indexOf('bbs') > -1 && location.href.indexOf('view') > -1){
				str = "?nttId=" + document.searchForm.nttId.value;
				data =  location.href + str;
			}
			if (IE) {
