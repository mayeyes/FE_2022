...
function tickerRotation() {
	...
	this.stop = function () {
        $("."+this.objName).find(" .b_roll_play > a").show().css("display","inline-block");
        $("."+this.objName).find(" .b_roll_stop > a").hide();
		clearInterval(this.GoodsSetTime);
	}
    this.play = function () {
        $("."+this.objName).find(" .b_roll_play > a").hide();
        $("."+this.objName).find(" .b_roll_stop > a").show().css("display","inline-block");
        if (this.autoScroll == 'none') {
			// do nothing.
		} else {
			this.scrollDirection = 'direction';
