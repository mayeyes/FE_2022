...
function type_visual_AC(){
	function _set(str){
		...
		if(str.find(" .midd>ul>li").length < 2){
			str.find(" .midd>ul>li").removeClass("on").eq(idx).addClass("on");
			return false;
		}
		for(var i=0; i<str.find(" .midd>ul>li").length; i++){
			c += '<a href="#"><span>'+(i+1)+'</span></a>';
		}
		$('<a href="#" class="btn_prev"><span>이전</span></a><a href="#" class="btn_next"><span>다음</span></a>').prependTo(str);
		$('<div class="controll">\
				'+c+'\
		...
		var obj = str;
			...
			obj.btn_prev = obj.find(" a.btn_prev");
			obj.btn_next = obj.find(" a.btn_next");
