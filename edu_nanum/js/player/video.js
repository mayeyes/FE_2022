...
var global_js_path = ""; 
//동영상 광고 스킵버튼 노출 
var global_ad_video_timeout = 10000; 
//플레이어 선언 
var global_player_set=""; 
//플레이어 설정 
var global_player_type=["html5","flash"];
//var global_player_type=["flash"];
//비디어 재생 시간 타임아웃
var global_save_timeout = 5000; 
//플래시 지원 버전 
//버전 체크는 비디오 개체가 생성되면 자동으로 html5로 넘어감 
//var global_flash_support_ver=11; 
var global_fullscreen_set=""; 
...
(function(window,undefined){
				var global_mouse_y = -1;
				var global_mouse_x = -1;
				var global_now_mouse_y = -1;
				var global_now_mouse_x = -1;
				var global_timer = 0; 
				var global_skip_timer = 0; 
				var global_image_ad_timer = -1; 
				var global_call_timer = -1; 	//5초마다 저장 
				var global_image_ad_count = 0; 
				var global_control_obj = null;
				var document=window.document;

				global_js_path = getActiveScript(); 
				...
				VideoJS.options={techOrder:global_player_type,
										  html5:{},
                                          flash:{swf:global_js_path+"hd_player_v27.swf"},
										  width:"auto",
										  height:"auto",
										  defaultVolume:0,
				...
				_V_.extend({
									...
									ua:navigator.userAgent,support:{},
									each:function(arr,fn){if(!arr||arr.length===0){return}
																for(var i=0,j=arr.length;i<j;i++){fn.call(this,arr[i],i)}},
									eachProp:function(obj,fn){if(!obj){return}
																		for(var name in obj){
																					if(obj.hasOwnProperty(name)){fn.call(this,name,obj[name])}}},
									...
									round:function(num,dec){if(!dec){dec=0}return Math.round(num*Math.pow(10,dec))/Math.pow(10,dec)},
									isEmpty:function(object){for(var prop in object){return false}return true},
									...
									getData:function(elem){
														...
														if(!id){id=elem[_V_.expando]=_V_.guid++;_V_.cache[id]={}}
														return _V_.cache[id]},
									removeData:function(elem){
															...
															try{delete elem[_V_.expando]}
																		...
																		if(elem.removeAttribute){elem.removeAttribute(_V_.expando)}
									...
									getAbsoluteURL:function(url){
																			var troublemaker = "\/\/";
																			if(!url.match(/^https?:troublemaker/)){
				...
				_V_.Component=_V_.Class.extend({
														...
														fadeOut:function(){this.removeClass("vjs-fade-in");this.addClass("vjs-fade-out")},
														lockShowing:function(){var style=this.el.style;style.display="block";style.opacity=1;style.visiblity="visible"},
														unlockShowing:function(){var style=this.el.style;style.display="";style.opacity="";style.visiblity=""},
				...
				_V_.ControlBar=_V_.Component.extend({/*컨트롤바 옵션설정*/
														options:{
																	loadEvent:"play",
																	components:{
																					playToggle:{},
																					...
																					timeDivider:{},
																					/*durationDisplay:{},*/
																					/*remainingTimeDisplay:{}, 영상 총 플레이 시간(진행시간을 표시하는 함수수정해서 포함시킴)*/
																					progressControl:{},
																					volumeControl:{},
																					muteToggle:{}
																	}
														...
														init:function(player,options){
																					...
																					player.addEvent("play",this.proxy(function(){
																																			...
																																			this.player.addEvent("mousemove",this.proxy(this.fadeIn));

																																			global_control_obj = this;

																																			this.player.addEvent("mouseout",this.proxy(this.fadeOut));
																					}));
														},
														createElement:function(){
																						return _V_.createElement("div",{className:"vjs-controls",id:"transact_video_control"})},
														fadeIn:function(){this._super();this.player.triggerEvent("controlsvisible")},
														fadeOut:function(){this._super();this.player.triggerEvent("controlshidden")},
				...
				_V_.Button=_V_.Control.extend({
														...
														createElement:function(type,attrs){
																									attrs=_V_.merge({
																															...
																															innerHTML:'<div><span class="vjs-control-text">'+(this.buttonText||"Need Text")+"</span></div>",role:"button",tabIndex:0
																												},attrs);
				...
				_V_.PauseButton=_V_.Button.extend({
														buttonText:"Pause",
														buildCSSClass:function(){return"vjs-pause-button "+this._super()},
				...
				_V_.PlayToggle=_V_.Button.extend({
														...
														if(global_control_state == 1) { alert("(1)Unable to control video playback time."); return; }	
				...
				//if(global_control_state == 1) { alert('(1)재생 제어가 불가능 합니다.'); return; }
				...
				//if(global_control_state == 1) { alert('(2)재생 제어가 불가능 합니다.'); return; }
				...
				_V_.Tful=_V_.Button.extend({//////////////////////////////////////////////////////풀스크린 경고
													init:function(player,options){
																		this._super(player,options);
															},
													createElement:function(){
																								...
																								return this._super("div",{
																													className:"vjs-tful",
																													innerHTML:"<span>Full Screen Mode 는 F11 키를 눌러주세요.</span>",
																													id:"transact_iefullscreen"});
																		}
...
//==새창으로 전체 띄우기 here 
				if(global_fullscreen_set != "") { newTransActwin(); return; }
				


																					if(transact_br_chk=="Internet Explorer"){
																							show_ad_image_pos_reset('full'); 
																							this.player.requestFullScreen();
																							document.getElementById("transact_iefullscreen").style.display ="block";
																							self.setTimeout("hideDiv()",3000);
																					}else if(transact_br_chk=="Internet Explorer11"){
																							show_ad_image_pos_reset('full'); 
																							if (document.getElementById('transact_video_1_html5_api').msRequestFullscreen) {
																									this.player.requestFullScreen();
																									document.getElementById('transact_video_1').msRequestFullscreen();
																							} else {
																									document.getElementById('transact_video_1').msExitFullscreen();
																							}
																					}else if(transact_br_chk==""){
																							show_ad_image_pos_reset('full'); 
																							document.getElementById("transact_iefullscreen").style.display ="none";
																							this.player.requestFullScreen();
																					}else{
																							show_ad_image_pos_reset('full'); 
																							this.player.requestFullScreen();
																					...
																					if(transact_br_chk=="Internet Explorer11"){
				...
				_V_.BigPlayButton=_V_.Button.extend({
																	...
																	createElement:function(){
																									return this._super("div",{
																																	...
																																	id:"big-center-playbtn", 
																																	innerHTML:"<span class='tooltiptext'>PLAY</span> "})
																						},
																	onClick:function(){
...
//																																	this.player.currentTime(_V_.formatTime(time,this.player.duration()))
//																											}
//																											this.player.play();
																				}
												});
		...
		_V_.Logo=_V_.Button.extend({//////////////////////////////////////////////////////로고 노출 innerhtml에 a태그 사용하여 링크 추가
													...
													//alert('logo2 popup'); 
														...
														return this._super("div",{
															className:"vjs-logo",
															innerHTML:"<span><a id='logo' href='#' onclick=\"\"><img id='logo_img' src='' width='' border='none'/><a/></span>",
															id:"transact_video_logo"
																																									});
													}
		...
		///광고 스킵버튼 노출 innerhtml에 a태그 사용하여 링크 추가
		_V_.Skipad=_V_.Button.extend({
													init:function(player,options){
																		this._super(player,options);
															},
													...
													createElement:function(){
														return this._super("div",{
															className:"vjs-skipad",
															innerHTML:"<span>Wait</span>",
															id:"transact_video_skipad"});
													}
													
						});


		//////////////////////////////////////////////////////이미지 광고 노출 innerhtml에 a태그 사용하여 링크 추가
		_V_.Adimg=_V_.Button.extend({
													...
													createElement:function(){
														return this._super("div",{
															className:"vjs-adimg",
															innerHTML:"<span> <div style='position:absolute; text-align:center; width:100%; z-index:90;'>  <a id='ad_img_link' href='#' onclick=\"\"><img id='ad_img_obj' border='none' alt='광고' /><a/> </div> <div id='closebtn_area' style='position:absolute; width:100%; z-index:100;cursor:pointer!important;'> <img id='ad_img_close_btn' border='none' src='' /> </div>  </span>",
															id:"transact_video_adimg"
																																									});
													}
				...
				_V_.Ad=_V_.Button.extend({//////////////////////////////////////////////////////인,아웃 트로시 광고중 문구 띄움
													init:function(player,options){
																		this._super(player,options);
															},
													createElement:function(){
													//alert('Ad popup!'); 
																								...
																								return this._super("div",{
																													className:"vjs-ad",
																													innerHTML:"<span>광고중</span>",
																													id:"transact_video_add"});
																		}
				...
				_V_.LoadingSpinner=_V_.Component.extend({
																			init:function(player,options){
																												...
																												player.addEvent("seeking",_V_.proxy(this,this.hide));
																												player.addEvent("error",_V_.proxy(this,this.hide));
				...
				_V_.CurrentTimeDisplay=_V_.Component.extend({
																				...
																				updateContent:function(){
																												var time=(this.player.scrubbing)?this.player.values.currentTime:this.player.currentTime();
																												//lhb global_image_ad_timer 
																												//if( this.player.duration()>3 ) global_image_ad_timer = 0; 

																												this.content.innerHTML=_V_.formatTime(time,this.player.duration())+" / "+_V_.formatTime(this.player.duration())
																										}	//진행시간 및 플레이 총 시간 표시
													});
				...
				_V_.RemainingTimeDisplay=_V_.Component.extend({
																						...
																						createElement:function(){
																															...
																															this.content=_V_.createElement("div",{className:"vjs-remaining-time-display",innerHTML:""});
																															el.appendChild(_V_.createElement("div").appendChild(this.content));
																															return el
																											},
																						updateContent:function(){
																															if(this.player.duration()){
																																							this.content.innerHTML="광고중";
																															}
																												}
															});*/
				_V_.Slider=_V_.Component.extend({
																...
																calculateDistance:function(event){
																											var box=this.el,boxX=_V_.findPosX(box),
		...
		if(global_control_state == 1) { alert("(2)Unable to control video playback time."); return; }	

		/*
		if(global_control_state == 2) { 

				console.log( 'this.player.duration()==>'+this.player.duration() ); 
				console.log( 'global_last_duration==>'+ global_last_duration ); 

				console.log( 'chk==>'+(this.player.duration()+1)  + '/' + (global_last_duration-1) ); 

				if( (this.player.duration()+1) > (global_last_duration-1) ) {
					alert("Info:You can only move before the current playback time."); 
					return;
				}
		...
		*/
		...
		if( global_last_duration < this.player.duration() ) global_last_duration = this.player.duration()+1; 


		var ad_disp = null;
		var ad_help_text = document.getElementById("transact_video_add");
		if(ad_help_text != null) ad_disp = ad_help_text.style.display;
		if(ad_disp == "block") { return; } 
		...
		if(global_control_state == 1) { alert("(3)Unable to control video playback time."); return; }
		...
		if(ad_disp == "block") { return; } 
				...
				_V_.PosterImage=_V_.Button.extend({
																	...
																	createElement:function(){return _V_.createElement("img",{
																																					className:"vjs-poster",
																																					src:this.player.options.poster,tabIndex:-1
																																					}
																																			)
																										},
																	onClick:function(){this.player.play()}
				...
				_V_.Menu=_V_.Component.extend({
																	...
																	addItem:function(component){
																											...
																											component.addEvent("click",this.proxy(
																																									function(){this.unlockShowing()}
																																									)
																				)},
																	createElement:function(){
				...
				_V_.Player=_V_.Component.extend({
																	init:function(tag,addOptions,ready){
																														...
																														el.className=tag.className;tag.id+="_html5_api";
																	...
																	destroy:function(){
																							...
																							_V_.players[this.id]=null;delete _V_.players[this.id];
																							this.tech.destroy();
																																			...
																																			for(var c,i=0,j=this.tag.childNodes;i<j.length;i++){
																																														...
																																														if(c.nodeName=="TRACK"){
																																																			options.tracks.push({
																																																											...
																																																											"default":c.getAttribute("default")!==null,title:c.getAttribute("title")
																																																			})
																																														}
																																			}
																											}
																	...
																	controlerhidden:function(){




																								nowtime = new Date();	
																								mstime = nowtime.getTime();
																								if( global_timer <= 0 ) global_timer = mstime; 
																								chk_timer = mstime-global_timer;
																								if(chk_timer <= 0) chk_timer = 0; 
																								if ( chk_timer>10000  ) {
																										global_timer = mstime;
																										if( global_mouse_x < global_now_mouse_x) //마우스 움직일때
																										{ 
																											global_mouse_x = global_now_mouse_x;
																											global_mouse_y = global_now_mouse_y;
																										} 
																										else if( global_mouse_x > global_now_mouse_x) //마우스 움직일때
																										{ 
																											global_mouse_x = global_now_mouse_x;
																											global_mouse_y = global_now_mouse_y;
																										} 
																											...
																											if(global_control_obj != null) global_control_obj.fadeOut(); //컨트롤객체 숨김
																											
																										}
																								}
																								p_chk_timer = chk_timer; 
		...
		if(ad_help_text != null) ad_disp = ad_help_text.style.display;
																								mstime = nowtime.getTime();
																								//동영상 광고 스킵 버튼 
																								if( global_skip_timer <= 0 ) global_skip_timer = mstime; 
																								chk_timer = mstime-global_skip_timer;
																								if(chk_timer <= 0) chk_timer = 0; 

																								if ( chk_timer>global_ad_video_timeout  && ad_disp != null) {
																										global_skip_timer = mstime;
																										if(ad_disp == "block") changecss('.vjs-default-skin div.vjs-skipad span','display','block'); 
																								}
																								...
																								//video playback save 
																								mstime = nowtime.getTime();
																								if( global_call_timer <= 0 ) global_call_timer = mstime; 
																								chk_timer = mstime-global_call_timer;
																								...
																								if ( chk_timer>global_save_timeout) {
																									var myVideo = document.getElementsByTagName('video')[0];

																									if(myVideo != null) {
			

				//if(global_last_duration < myVideo.currentTime) global_last_duration = myVideo.currentTime; 

																									var ctime = myVideo.currentTime; 
																										if(ctime != null) {
																											var nowtime = parseInt(ctime);
																																																	//한계시간 도달하면 저장 처리 
																											//alert('time==>'+ctime); 
																											SaveVideoPlayBack( nowtime ); 

				global_call_timer = mstime;
																										}
																								...
																								var img_obj_width = 0; 
																								if(img_obj != null) img_obj_width = img_obj.width; 

																								if(ad_disp != "block" && global_image_ad_timer==-1 && p_chk_timer>100 && img_obj_width>1) { global_image_ad_timer = 0; } 
																								//이미지 배너 광고 노출 시간 global_image_ad_timer
																								if( global_image_ad_timer == 0 ) global_image_ad_timer = mstime; 
																								chk_timer = 0; 
																								if(global_image_ad_timer>0) chk_timer = mstime-global_image_ad_timer;

																								if ( (chk_timer>4000 && chk_timer<14000 ) && ad_disp != "block" && global_image_ad_timer>0 && global_image_ad_count<1) {

			//		alert('chk_timer-->'+ chk_timer + '/global_image_ad_timer==>'+ global_image_ad_timer); 
																									//global_image_ad_timer = -100; 
																									global_image_ad_count = global_image_ad_count + 1; 
																									changecss('.vjs-default-skin div.vjs-adimg span','display','block');
																								}


																								if ( chk_timer>15000  && ad_disp != "block" ) {
					//alert('ad img stop!'); 
																									global_image_ad_timer = -1000;
																	...
																	onPlay:function(){
																						_V_.removeClass(this.el,"vjs-paused");
																						_V_.addClass(this.el,"vjs-playing")},
																	onPause:function(){_V_.removeClass(this.el,"vjs-playing");_V_.addClass(this.el,"vjs-paused")},
																	onProgress:function(){

																		if(this.bufferedPercent()==1){this.triggerEvent("loadedalldata")}
																	...
																	play:function(){this.techCall("play");
																						return this},
																	pause:function(){this.techCall("pause");return this},
																	paused:function(){return(this.techGet("paused")===false)?false:true},
																	currentTime:function(seconds){

																									this.addEvent("mousemove",function(e){//마우스 좌표 가져옴
																										global_now_mouse_x = Math.round (e.clientX);
																										global_now_mouse_y = Math.round (e.clientY);
																									});

																									this.controlerhidden(); //controlerhidden함수 호출(일정시간마다 마우스 좌표 검사 및 컨트롤바 숨김기능)
																	...
																	enterFullWindow:function(){
																										this.isFullWindow=true;this.docOrigOverflow=document.documentElement.style.overflow;
																										_V_.addEvent(document,"keydown",_V_.proxy(this,this.fullWindowOnEscKey));


//	var cl = document.getElementById("transact_video_control");
//		cl.addEvent(document,"keydown",_V_.proxy(this,this.fullWindowOnEscKey));
					...
					if(global_control_obj != null) global_control_obj.fadeIn(); 
					...
					if(event.keyCode== 38) {
						var myVideo = _V_("transact_video_1");
						myVideo.muted(false); myVideo.volume(myVideo.volume() + 0.1);
					}
					if(event.keyCode== 40) {
						var myVideo = _V_("transact_video_1");
					...
					if(event.keyCode== 9) {
						...
						_V_.addClass(cl, "vjs-fade-in");
								...
								//techName="flash";
																	...
																	load:function(){this.techCall("load");return this},
																	currentSrc:function(){return this.techGet("currentSrc")||this.values.src||""},
																	preload:function(value){if(value!==undefined){this.techCall("setPreload",value);this.options.preload=value;return this}return this.techGet("preload")},
																	autoplay:function(value){if(value!==undefined){this.techCall("setAutoplay",value);this.options.autoplay=value;return this}return this.techGet("autoplay",value)},
																	loop:function(value){if(value!==undefined){this.techCall("setLoop",value);this.options.loop=value;return this}return this.techGet("loop")},
						...
						function(){
										...
										if(requestFn){
														_V_.support.requestFullScreen={
																								requestFn:requestFn,cancelFn:cancelFn,eventName:eventName,isFullScreen:isFullScreen
																								}
										}
				})();
				...
				if(global_control_state == 1) { alert("(4)Unable to control video playback time."); return; }
				...
				_V_.html5=_V_.PlaybackTech.extend({
																	...
																	eventHandler:function(e){e.stopPropagation();this.triggerEvent(e)},
																	play:function(){this.el.play()},
																	pause:function(){this.el.pause()},
																	paused:function(){return this.el.paused},
																	currentTime:function(){return this.el.currentTime},
																	setCurrentTime:function(seconds){try{this.el.currentTime=seconds}catch(e){_V_.log(e,"Video isn't ready. (VideoJS)")}},
																	...
																	supportsFullScreen:function(){
																											if(typeof this.el.webkitEnterFullScreen=="function"){
																																			if(!navigator.userAgent.match("Chrome")&&!navigator.userAgent.match("Mac OS X 10.5")){return true}
																											}
																											return false
																								},
																	enterFullScreen:function(){try{this.el.webkitEnterFullScreen()}catch(e){if(e.code==11){_V_.log("VideoJS: Video not ready.")}}},
				...
				_V_.flash=_V_.PlaybackTech.extend({
																init:function(player,options){
																									...
																									var source=options.source,
																											...
																											flashVars=_V_.merge({readyFunction:"_V_.flash.onReady",
																																			eventProxyFunction:"_V_.flash.onEvent",
																																			errorEventProxyFunction:"_V_.flash.onError",
																																			autoplay:playerOptions.autoplay,
																																			preload:playerOptions.preload,
																																			loop:playerOptions.loop,
																																			muted:playerOptions.muted},options.flashVars),
																									...
																									if(options.iFrameMode==true&&!_V_.isFF){
																															var iFrm=_V_.createElement("iframe",{id:objId+"_iframe",
																																													name:objId+"_iframe",
																																													className:"vjs-tech",
																																													scrolling:"no",
																																													marginWidth:0,
																															...
																															_V_.addEvent(iFrm,"load",_V_.proxy(this,function(){
																																																...
																																																iWin.ready=_V_.proxy(this.player,function(currSwf){
																																																																var el=iDoc.getElementById(currSwf),player=this,tech=player.tech;tech.el=el;
				...
				_V_.flash.prototype.support={formats:{"video/flv":"FLV","video/x-flv":"FLV","video/mp4":"MP4","video/m4v":"MP4"},
															progressEvent:false,
				...
				_V_.flash.checkReady=function(tech){
																if(tech.el.vjs_getProperty){tech.triggerReady()}
																else{setTimeout(function(){_V_.flash.checkReady(tech)},50)}};
				_V_.flash.onEvent=function(swfID,eventName){var player=_V_.el(swfID).player;player.triggerEvent(eventName)};
				_V_.flash.onError=function(swfID,err){var player=_V_.el(swfID).player;player.triggerEvent("error");_V_.log("Flash Error",err,swfID)};
				...
				_V_.flash.embed=function(swf,placeHolder,flashVars,params,attributes){
																							...
																							if(_V_.isIE()){
																										...
																										setTimeout(function(){
																																		newObj.style.display="block"},1000)
																							}
																							return obj};
				...
				_V_.Track=_V_.Component.extend({init:function(player,options){
																									this._super(player,options);
																									_V_.merge(this,{id:options.id||("vjs_"+options.kind+"_"+options.language+"_"+_V_.guid++),
																															src:options.src,
																															"default":options["default"],
																															title:options.title,
																															language:options.srclang,
																															label:options.label,cues:[],
																															activeCues:[],
																															readyState:0,mode:0
																													})
																					},
																...
																show:function(){this.activate();this.mode=2;this._super()},
																hide:function(){this.activate();this.mode=1;this._super()},
																disable:function(){if(this.mode==2){this.hide()}this.deactivate();this.mode=0},
																activate:function(){
																...
																onError:function(err){this.error=err;this.readyState=3;this.triggerEvent("error")},
																parseCues:function(srcContent){
																									...
																									for(var i=1,j=lines.length;i<j;i++){
																																...
																																if(line){
																																		...
																																		cue={id:id,index:this.cues.length};time=line.split(" --> ");
				...
				_V_.OffTextTrackMenuItem=_V_.TextTrackMenuItem.extend({init:function(player,options){
																										...
																										onClick:function(){this._super();this.player.showTextTrack(this.track.id,this.track.kind)},
																										update:function(){
				...
				_V_.TextTrackButton=_V_.Button.extend({init:function(player,options){
																			...
																			onBlur:function(){},
																			onClick:function(){this.one("mouseout",this.proxy(function(){
				...
				_V_.ChaptersButton=_V_.TextTrackButton.extend({kind:"chapters",
																						buttonText:"Chapters",
