...
//진행바
/*var obj = $('#section1 [data-main="01_01"] .control');
    $("<div class='progress'><span></span><span></span></div>").appendTo(obj);
    slide.controls.find(".progress span").css({ "width":"0" });
    slide.controls.find(".progress span:first-child").animate({
        "width" : "100%"
    },3750);
    setTimeout(function(){
		slide.controls.find(".progress span:nth-child(2)").animate({
            "width" : "100%"
        },3750);
	},3750);*/
    
$("<div class='progress'><span><i></i></span><span><i></i></span></div>").appendTo(slide.controls);
...
slide.btn_stop.click(function(){
	...
	slide.autos = "N";
	clearTimeout(slide.times);
    slide.controls.find(".progress span:nth-child(1) i").stop();
	return false;
});
...
function movement(ty){
	...
	if(ty == "right"){
		...
		slide.controls.find(".progress span i").css({ "width":"0" });
        slide.controls.find(".progress span:nth-child(1) i").animate({
            "width" : "100%"
        },slide.times_speeds);
        /*slide.controls.find(".progress span:nth-child(2)").delay(3750).animate({
            "width" : "100%"
        },3750);*/
		//alert(news);
...
function board_list_secret_AC(){
	var obj = $(".qna_list a.js_secret");
	var htmls = '';
		htmls += '<div class="js_password_box">';
		htmls += '	<div class="wbx">';
		htmls += '		<strong>알림</strong>';
		htmls += '		<div>';
		htmls += '			<span>비밀번호를 입력해주세요</span>';
		htmls += '			<div data-skin="password">';
		htmls += '				<input type="password" name="sct_password" id="sct_password" placeholder="숫자 4자리" />';
		htmls += '			</div>';
		htmls += '			<div class="btnbx">';
		htmls += '				<a href="#" class="no">취소</a><a href="#" class="ok" onclick="board_secret_form();">확인</a>';
		htmls += '			</div>';
		htmls += '		</div>';
	...
	obj.click(function(){
		if($(this).hasClass("js_secret") && !obj.find(" .js_secret").is(":hidden")){
			pw_box.fadeIn(200);
			pw_blind.fadeIn(200);
			//검색
	...
	pw_box.find(" .btnbx>a").click(function(){
		pw_box.fadeOut(200);
		pw_blind.fadeOut(200);
		return false;
	});
	pw_box.find(" .btn_close").click(function(){
		pw_box.fadeOut(200);
		pw_blind.fadeOut(200);
		return false;
	});
