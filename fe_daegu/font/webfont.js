(function () {
    ...
    if (
        (window.localStorage && localStorage.daegu_font_css_cache)
        || document.cookie.indexOf('daegu_font_css_cache') > -1
    ) {
        // 캐시된 버전을 사용한다.
    ...
    function isFileCached(href) {
        return (
            window.localStorage
            && localStorage.daegu_font_css_cache
            && (localStorage.daegu_font_css_cache_file === href)
        );
    }
    ...
    function injectFontsStylesheet() {
        ...
        if (isOldBrowser()) {
            ...
            document.cookie = "daegu_font_css_cache";
        } else {
            // 로컬 스토리지에 캐시한 버전이 있는경우
            injectRawStyle(localStorage.daegu_font_css_cache);
        }
    }
        ...
        setTimeout(function(){
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    ...
                    // 기존에 저장된 것이 있다면 덮어쓴다
                    localStorage.daegu_font_css_cache = xhr.responseText;
                    localStorage.daegu_font_css_cache_file = css_href;
                }
            };
            xhr.send();
        },500);
